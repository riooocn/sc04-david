...
export default function Index({ products }) {
    ...
    const deleteItem = () => {
        destroy(route('admin.products.destroy', deletingId), {
            preserveScroll: true,
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 bg-white border-b border-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {products.data.map((product) => (
                                        <tr key={product.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(product.price)}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{product.stock}</td>
                                            <td className="px-6 py-4 text-sm text-gray-500">
                                                {product.categories.map(c => <span key={c.id} className="inline-block bg-gray-200 rounded-full px-2 py-1 text-xs font-semibold text-gray-700 mr-1">{c.name}</span>)}
                                            </td>
                            ...
                            <div className="mt-4">
                                {/* Simple Pagination Links placeholder */}
                                <div className="flex gap-2">
